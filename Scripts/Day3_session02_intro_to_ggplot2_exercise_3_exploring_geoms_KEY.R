# Exercise: Day3_session02_intro_to_ggplot2_exercise_3_exploring_geoms_KEY
...
# columns
df_tst_psnu %>%
  ggplot(aes(x = fy, y = cumulative, fill = psnu)) +
  geom_col()
...
# points (scatterplot)
df_tst_psnu %>%
  ggplot(aes(x = fy, y = cumulative, group = psnu, size = psnu)) +
  geom_line()
...
df_tst_psnu %>% 
  ...
  facet_wrap(~ psnu) + si_style_ygrid()
