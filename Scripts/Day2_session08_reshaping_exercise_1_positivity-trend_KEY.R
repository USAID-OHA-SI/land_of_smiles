# Day2_session08_reshaping_exercise_1_positivity-trend_KEY.R
...
df_long <- df_semi %>% 
  pivot_longer(starts_with("qtr"), #name columns of interest (qtr1-4)
               names_to = "quarter", #new column name 
               names_prefix = "qtr", #remove text from variable
               values_to = "value") #new value name
...
df_long2 <- df_long %>% 
  pivot_wider(names_from = "indicator", #spread indicator column
              values_from = "value") #name of value column
