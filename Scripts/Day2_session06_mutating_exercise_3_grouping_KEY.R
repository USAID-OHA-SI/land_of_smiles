# Exercise: Day2_session06_mutating_exercise_3_grouping_KEY.R
...
df_index_psnu_trend %>% 
  mutate(achievement = cumulative / targets, 
         targets_prior = lag(targets), #lag calculation for full dataset
         max_targets = max(targets)) %>% #max calculation for full dataset
