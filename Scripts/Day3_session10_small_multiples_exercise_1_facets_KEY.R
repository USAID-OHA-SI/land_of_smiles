# Exercise: Day3_session10_small_multiples_exercise_1_facets_KEY
...
df_tst_psnu %>% 
  ...
  facet_grid(snu1 ~ psnu) #same axes for subplots 
...
df_tst_psnu %>% 
  ...
  geom_col(width = 0.5) + 
  labs(title = "TESTING RESULTS") +
  facet_grid( ~psnu) #same axes for psnu subplots 
...
df_tst_psnu %>% 
  ...
  geom_col(width = 0.5) + 
  labs(title = "TESTING RESULTS") +
  facet_wrap( snu1~psnu ) #different axes for subplots
...
df_tst_psnu %>% 
  ...
  labs(title = "TESTING RESULTS") +
  facet_wrap( ~psnu, 
              scales = "free_y", #default is "fixed", free scale on y-axis
              strip.position = "bottom") #default is "top", moves labels down  


# Review the help sections for facet_grid() and facet_wrap()
?facet_grid #produces a grid of plots for each combination of variables
?facet_wrap #only produces plots for the combination of variables that have values
